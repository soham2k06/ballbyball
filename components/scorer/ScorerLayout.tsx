    ...
    <Card className='max-sm:w-full sm:w-96 border-0'>
      ...
      <CardContent className='space-y-2 p-0'>
        <div className='flex gap-1 justify-center w-full'>
          {['0', '1', '2', '3'].map((event, i) => (
            <Button key={i} className='w-full h-16 text-lg' value={event} onClick={handleScore}>
              {event}
            </Button>
          ))}
        </div>
      ...
      <CardFooter className='block'>
        <div className='gap-2 flex pb-6'>
          ...
          />
          <Button size='sm' variant='destructive' className='w-full' onClick={handleUndo}>
            Undo
          </Button>
        </div>
        <div className='bg-muted text-primary w-full left-0 rounded-md flex items-center p-2'>
          <span>Extras: {extras.length}</span>
          <Separator orientation='vertical' className='bg-muted-foreground h-6 mx-3' />
          <span>
            Current Over: {curOverRuns || 0}/
            {overSummaries[curOverIndex]?.filter((ball) => ball === '-1').length || 0}
          </span>
        </div>
      </CardFooter>
    </Card>
